...
|
| Role gates follow spec §7: ADMIN passes everything, HR gets the operational
| modules, EMPLOYEE only reaches its own attendance and profile.
|
*/

$hr = 'role:'.Role::ADMIN.','.Role::HR;
...
Route::middleware(['auth', 'active'])->group(function () use ($hr, $admin) {
    ...
    |----------------------------------------------------------------------
    */
    Route::prefix('attendance')->name('attendance.')->group(function () use ($hr) {
        // Static segments must be declared before /{attendance}.
        Route::get('/history', [AttendanceController::class, 'history'])->name('history');
        Route::get('/monitoring', [AttendanceController::class, 'monitoring'])->middleware($hr)->name('monitoring');
